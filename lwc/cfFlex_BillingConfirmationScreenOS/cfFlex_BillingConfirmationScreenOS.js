import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfFlex_BillingConfirmationScreenOS extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @track omniSupportKey = 'cfFlex_BillingConfirmationScreenOS';
              @track record;
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-a5P5f000000QKqJEAI"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5f00000OIC8pEAP").then(() =>{
                    //Loading Custom CSS.
                  })
                });
