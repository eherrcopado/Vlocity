import { FlexCardMixin } from "vlocity_ins/flexCardMixin";
    import {interpolateWithRegex, interpolateKeyValue, fetchCustomLabels } from "vlocity_ins/flexCardUtility";
          ...
          export default class cfFlex_PaymentConfirmationScreenOS extends FlexCardMixin(OmniscriptBaseMixin(LightningElement)){
              @api debug;
              @api recordId;
              @track omniSupportKey = 'cfFlex_PaymentConfirmationScreenOS';
              @track record;
              ...
              connectedCallback() {
                ...
                this.setAttribute(
                  "class",
                  "card-a5P5f000000QKqIEAI"
                );
                this.getCommunityPrefix().then((result) => {
                  loadStyle(this, result +"/servlet/servlet.FileDownload?file=00P5f00000OIC8oEAH").then(() =>{
                    //Loading Custom CSS.
                  })
                });
